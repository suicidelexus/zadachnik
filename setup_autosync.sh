...
if launchctl list | grep -q "com.zadachnik.autosync"; then
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║  ✅ УСПЕШНО! Ежедневная автосинхронизация настроена         ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo ""
    echo "📊 Что это означает:"
    echo "   • Сервис запущен и работает в фоне"
    echo "   • Автоматически синхронизирует каждый день в 18:00"
    echo "   • Сначала получает изменения с GitHub (pull)"
    echo "   • Затем отправляет ваши изменения (commit + push)"
    ...
    echo ""
    echo "💾 Принудительная синхронизация СЕЙЧАС:"
    echo "   ./s"
    echo ""
    echo "🎉 Теперь изменения автоматически синхронизируются каждый день в 18:00!"
    echo ""
else
    echo ""
